...
const moveToSuccess = (LicenseData, dispatch) => {
  ...
  dispatch(
    setRoute(
      `/parking/acknowledgement?purpose=${purpose}&status=${status}&applicationNumber=${applicationNo}&FY=${financialYear}&tenantId=${tenantId}`
    )
  );
};
const editRenewalMoveToSuccess = (LicenseData, dispatch) => {
  ...
  dispatch(
    setRoute(
      `/parking/acknowledgement?purpose=${purpose}&status=${status}&applicationNumber=${applicationNo}&licenseNumber=${licenseNumber}&FY=${financialYear}&tenantId=${tenantId}`
    )
  );
};
  ...
  if (activeStep === 2) {
    ...
    if (isFormValid) {
      if (getQueryArg(window.location.href, "action") === "edit") {
        ...
        dispatch(
          setRoute(
            `/parking/search-preview?applicationNumber=${businessId}&tenantId=${tenantId}&edited=true`
          )
        );
        const updateMessage = {
...
export const renewparking = async (financialYear, state, dispatch) => {
  const licences = get(
    state.screenConfiguration.preparedFinalObject,
    `Licenses`
  );
  ...
  dispatch(
    setRoute(
      `/parking/acknowledgement?purpose=EDITRENEWAL&status=success&applicationNumber=${renewedapplicationNo}&licenseNumber=${licenseNumber}&FY=${nextFinancialYear}&tenantId=${tenantId}&action=${wfCode}`
    ));
};
          ...
          children: {
            ...
            submitButton: {
              ...
              onClickDefination: {
                action: "condition",
                callBack: () => {
                  renewparking(financialYear, state, dispatch);
                },
