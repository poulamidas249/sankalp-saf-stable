...
import { getSearchResultsForSafFormat, getSearchResultsForSafMaster, getSearchResultsForSafList } from "../../../../ui-utils/commons";
...
const searchApiCall = async (state, dispatch, index) => {
  ...
  let formValid = false;
  if (searchScreenObject.assessmentNumber != "" && searchScreenObject.acknowledgementNumber != "") {
    formValid = true;
  }
  if (!formValid) {
    dispatch(
      toggleSnackbar(
        true,
        {
          labelName: "Please fill both fields to search",
          labelKey: "Please fill both fields to search"
        },
        "error"
      )
    );
  ...
  if (
    ...
    try {
      disableField('safSearch', "components.div.children.searchApplicationDetails.children.cardContent.children.appNumberContainer.children.searchButton", dispatch);
      ...
      const response = await getSearchResultsForSafMaster(searchScreenObject.acknowledgementNumber, searchScreenObject.assessmentNumber, queryObject);
      const safListResponse = await getSearchResultsForSafList(searchScreenObject.acknowledgementNumber, searchScreenObject.assessmentNumber, queryObject);
      ...
      let applicationData = safListResponse.safList.map(item => ({
        ["SAF Number"]:
          item.safNo || "-",
        ["Assessee Number"]:
          item.assesseeNo || "-",
      ...
      enableField('safSearch', "components.div.children.searchApplicationDetails.children.cardContent.children.appNumberContainer.children.searchButton", dispatch);
      dispatch(
        handleField(
          "safSearch",
      ...
      //showHideProgress(false, dispatch);
      enableField('safSearch', "components.div.children.propertySearchTabs.children.cardContent.children.tabSection.props.tabs[0].tabContent.searchPropertyDetails.children.cardContent.children.button.children.buttonContainer.children.searchButton", dispatch);
      enableField('safSearch', "components.div.children.propertySearchTabs.children.cardContent.children.tabSection.props.tabs[1].tabContent.searchApplicationDetails.children.cardContent.children.button.children.buttonContainer.children.searchButton", dispatch);
      dispatch(
        toggleSnackbar(
          true,
