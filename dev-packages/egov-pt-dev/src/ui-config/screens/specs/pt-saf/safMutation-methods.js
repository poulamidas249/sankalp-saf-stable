...
import { applicationSearch, propertySearch } from "./safFunctions";
...
const applyMohallaData = (mohallaData, tenantId, dispatch) => {
  ...
  dispatch(
    handleField("safSearch", ComponentJsonPath.street, "props.value", "")
  );
  dispatch(
    handleField("safSearch", ComponentJsonPath.street, "props.error", false)
  );
  dispatch(
    handleField("safSearch", ComponentJsonPath.street, "isFieldValid", true)
  );
  dispatch(
    handleField("safSearch", ComponentJsonPath.street, "props.errorMessage", "")
  );
  dispatch(
    handleField("safSearch", ComponentJsonPath.street, "props.helperText", "")
  );
  dispatch(
    handleField("safSearch", ComponentJsonPath.ward, "props.helperText", "")
  );
  dispatch(
    handleField("safSearch", ComponentJsonPath.ward, "props.error", false)
  );
  dispatch(
    handleField("safSearch", ComponentJsonPath.ward, "props.isFieldValid", true)
  );
    ...
    ownerName: getTextField({
      ...
      afterFieldChange: async (action, state, dispatch) => {
        if (action.value.match(/^[^{0-9}^\$\"<>?\\\\~!@#$%^()+={}\[\]*,/_:;“”‘’]{3,50}$/i) || action.value.length == 0) {
          dispatch(
            handleField("safSearch", ComponentJsonPath.ownerName, "props.error", false)
          );
          dispatch(
            handleField("safSearch", ComponentJsonPath.ownerName, "isFieldValid", true)
          );
          dispatch(
            handleField("safSearch", ComponentJsonPath.ownerName, "props.errorMessage", "")
          );
        } else {
          dispatch(
            handleField("safSearch", ComponentJsonPath.ownerName, "props.error", true)
          );
          dispatch(
            handleField("safSearch", ComponentJsonPath.ownerName, "isFieldValid", false)
          );
          dispatch(
            handleField("safSearch", ComponentJsonPath.ownerName, "props.errorMessage", action.value.length < 3 ? getLocaleLabels("PT_ERR_MIN3CHAR", "PT_ERR_MIN3CHAR") : getLocaleLabels("PT_ERR_INVALID_TEXT", "PT_ERR_INVALID_TEXT"))
          );
        }
      }
    }),
...
export const searchApplicationDetails = getCommonCard({
  ...
  subParagraph: getCommonParagraph({
    labelName: "Provide at least one non-mandatory parameter to search for an application",
    labelKey: "Provide at least one non-mandatory parameter to search for an application"
  }),
  appNumberContainer: getCommonContainer({
    propertyAcknowledgementNumber: getTextField({
      label: {
        labelName: "Acknowledgement Number",
        labelKey: "Acknowledgement Number"
      },
      placeholder: {
        labelName: "Enter Acknowledgement Number",
        labelKey: "Enter Acknowledgement Number"
      },
      gridDefination: {
        xs: 12,
        sm: 4,
      ...
      required: false,
      errorMessage: "Invalid Assessment Number",
      jsonPath: "ptSearchScreen.acknowledgementNumber"
    }),
