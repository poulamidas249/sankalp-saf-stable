...
const applicationNumberClick = async (item) => {
  ...
  let isView = getQueryArg(window.location.href, "isView")
  if (isView) {
    navigate(`/property-tax/saf-preview?applicationNumber=${item.assesseeNo}&tenantId=${item.owners[0].tenantId}&type=property&isView=true`);
  } else {
    navigate(`/property-tax/saf-preview?applicationNumber=${item.assesseeNo}&tenantId=${item.owners[0].tenantId}&type=property`);
  }
