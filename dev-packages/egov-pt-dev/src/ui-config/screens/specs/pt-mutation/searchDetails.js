...
const applicationNumberClick = async () => { 
  window.location.href = `../pt-mutation/hearingNotice`;
};
...
const assesseeNumberClick = async (item) => {
  ...
  // const businessService = await getApplicationType(item && item.assesseeNo, item.tenantId, item.creationReason);
  // if (businessService == 'PT.MUTATION') {
  //   navigate(`/pt-mutation/hearingNotice?applicationNumber=${item.assesseeNo}`);
  // } else if (businessService == 'PT.CREATE') {
  //   navigate(`/property-tax/hearingNotice?applicationNumber=${item.assesseeNo}`);
