  ...
  props: {
    ...
    columns: [
          ...
          customBodyRender: (value, tableMeta) => (
            <div>
              <a
                ...
                onClick={() => onPropertyTabClick(tableMeta)}
              >
                {value}
              </a>
            </div>
      ...
      { labelName: "Guardian Name", labelKey: "PT_GUARDIAN_NAME" },
      {
        ...
        labelKey: "PT_COMMON_COL_EXISTING_PROP_ID",
      },
...
export const searchApplicationTable = {
  ...
  props: {
    ...
    columns: [
      ...
      },
      {
        ...
        options: {
          filter: false,
          customBodyRender: (value) => (
            <a href="javascript:void(0)" onClick={() => propertyIdClick(value)}>
              {value.propertyId}
            </a>
          ),
        },
      },
      {
        ...
        labelKey: "PT_COMMON_TABLE_COL_APP_TYPE",
      },
      { labelName: "Owner Name", labelKey: "PT_COMMON_TABLE_COL_OWNER_NAME" },
      { labelName: "Address", labelKey: "PT_COMMON_COL_ADDRESS" },
      {
    ...
    title: {
      labelKey: "PT_HOME_APPLICATION_RESULTS_TABLE_HEADING",
      labelName: "Search Results for Property Application",
    },
    rows: "",
    options: {
      filter: false,
...
const applicationNumberClick = async (item) => {
  const businessService = await getApplicationType(
    item && item.acknowldgementNumber,
    item.tenantId,
    item.creationReason
  );
  if (businessService == "PT.MUTATION") {
    navigate(
      `/pt-mutation/search-preview?applicationNumber=${item.acknowldgementNumber}&tenantId=${item.tenantId}`
    );
  } else if (businessService == "PT.CREATE") {
    navigate(
      `/property-tax/application-preview?applicationNumber=${item.acknowldgementNumber}&tenantId=${item.tenantId}&type=property`
    );
  } else if (businessService == "PT.LEGACY") {
    navigate(
      `/property-tax/application-preview?applicationNumber=${item.acknowldgementNumber}&tenantId=${item.tenantId}&type=legacy`
    );
  } else if (businessService == "PT.UPDATE") {
    navigate(
      `/property-tax/application-preview?applicationNumber=${item.acknowldgementNumber}&tenantId=${item.tenantId}&type=updateProperty`
    );
  } else {
