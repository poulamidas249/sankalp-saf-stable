  ...
  componentDidMount = async () => {
    ...
    try {
      ...
      if (get(pgUpdateResponse, "Transaction[0].txnStatus") === "FAILURE") {
        ...
        }/parking/acknowledgement?purpose=${"pay"}&status=${"failure"}&applicationNumber=${consumerCode}&FY=${financialYear}&tenantId=${tenantId}`;
      } else {
        let transactionId = get(pgUpdateResponse, "Transaction[0].txnId");
        window.location.href = `${
          process.env.NODE_ENV === "production" ? "/citizen" : ""
        }/parking/acknowledgement?purpose=${"pay"}&status=${"success"}&applicationNumber=${consumerCode}&FY=${financialYear}&tenantId=${tenantId}&secondNumber=${transactionId}`;
      }
    } catch (e) {
      alert(e);
    }
