...
const moveToSuccess = (href, dispatch, receiptNumber) => {
  ...
  dispatch(
    setRoute(
      `/advertising/acknowledgement?purpose=${purpose}&status=${status}&applicationNumber=${applicationNo}&tenantId=${tenantId}&secondNumber=${receiptNumber}`
    )
  );
};
...
const callBackForPay = async (state, dispatch) => {
  ...
  const { selectedTabIndex, selectedPaymentMode, fieldsToValidate } =
