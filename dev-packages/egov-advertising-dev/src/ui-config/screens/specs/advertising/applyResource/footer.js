...
const moveToSuccess = (LicenseData, dispatch) => {
  ...
  dispatch(
    setRoute(
      `/advertising/acknowledgement?purpose=${purpose}&status=${status}&applicationNumber=${applicationNo}&FY=${financialYear}&tenantId=${tenantId}`
    )
  );
};
const editRenewalMoveToSuccess = (LicenseData, dispatch) => {
  ...
  dispatch(
    setRoute(
      `/advertising/acknowledgement?purpose=${purpose}&status=${status}&applicationNumber=${applicationNo}&licenseNumber=${licenseNumber}&FY=${financialYear}&tenantId=${tenantId}`
    )
  );
};
  ...
  if (activeStep === 2) {
    ...
    if (isFormValid) {
      if (getQueryArg(window.location.href, "action") === "edit") {
        ...
        dispatch(
          setRoute(
            `/advertising/search-preview?applicationNumber=${businessId}&tenantId=${tenantId}&edited=true`
          )
        );
        const updateMessage = {
      ...
      uploadedDocData = uploadedDocData.filter(
...
export const renewadvertising = async (financialYear, state, dispatch) => {
  const licences = get(
    state.screenConfiguration.preparedFinalObject,
    `Licenses`
  );
  ...
  dispatch(
    setRoute(
      `/advertising/acknowledgement?purpose=EDITRENEWAL&status=success&applicationNumber=${renewedapplicationNo}&licenseNumber=${licenseNumber}&FY=${nextFinancialYear}&tenantId=${tenantId}&action=${wfCode}`
    )
  );
};
          ...
          children: {
            ...
            submitButton: {
              ...
              onClickDefination: {
                action: "condition",
                callBack: () => {
                  renewadvertising(financialYear, state, dispatch);
                },
              },
              visible:
                (getButtonVisibility(status, "APPROVED") ||
...
export const footerReviewTop = (
  ...
  let applicationDownloadObject = {
    ...
    link: () => {
      const { Licenses, LicensesTemp } =
        state.screenConfiguration.preparedFinalObject;
      const documents = LicensesTemp[0].reviewDocData;
      set(Licenses[0], "additionalDetails.documents", documents);
      generateTLAcknowledgement(
        state.screenConfiguration.preparedFinalObject,
        `tl-acknowledgement-${Licenses[0].applicationNumber}`
      );
    },
    leftIcon: "assignment",
  };
