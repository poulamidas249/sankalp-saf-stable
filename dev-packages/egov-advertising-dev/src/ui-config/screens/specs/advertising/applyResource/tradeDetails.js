...
const tradeCategoryChange = (reqObj) => {
  try {
    ...
    dispatch(
      pFO(`Licenses[0].tradeLicenseDetail.tradeUnits[${index}].tradeType`, "")
    );
  } catch (e) {
    console.log(e);
  }
...
const tradeSubTypeChange = (reqObj) => {
  try {
    let { moduleName, rootBlockSub, keyValue, value, state, dispatch, index } =
      reqObj;
    ...
    if (currentObject[0] && currentObject[0].uom !== null) {
      ...
      dispatch(
        pFO(`Licenses[0].tradeLicenseDetail.tradeUnits[${index}].uom`, null)
      );
      dispatch(
        pFO(
    ...
    dispatch(
      pFO(
        `Licenses[0].tradeLicenseDetail.tradeUnits[${index}].tradeType`,
        value
      )
    );
  } catch (e) {
...
const structureSubTypeChange = (reqObj) => {
  try {
    ...
    dispatch(
      pFO("LicensesTemp[0].tradeLicenseDetail.structureType", keyValueRow)
    );
  } catch (e) {
    console.log(e);
  }
        ...
        accessoriesName: {
          ...
          beforeFieldChange: (action, state, dispatch) => {
            try {
              ...
              let currentUOMValueFieldPath =
                action.componentJsonpath.split(".");
        ...
        accessoriesUOMValue: {
          ...getTextField({
            ...
            props: {
              ...
              disabled: true,
              jsonPath:
                "Licenses[0].tradeLicenseDetail.accessories[0].uomValue",
            },
            required: true,
...
export const tradeDetails = getCommonCard({
  ...
  tradeDetailsConatiner: getCommonContainer(
    ...
    },
    {
      style:
        getQueryArg(window.location.href, "action") === "EDITRENEWAL" ||
        getQueryArg(window.location.href, "workflowService") === "EDITRENEWAL"
          ? { cursor: "not-allowed" }
          : {},
    }
  ),
  ...
  for (var variable in multiItemContent) {
    ...
    if (multiItemContent[variable].props.setDataInField && value) {
      if (
        ...
        if (data) {
          multiItemContent["tradeType"].props.data =
            this.objectToDropdown(data);
        }
      } else if (
        multiItemContent[variable].props.jsonPath.split(".").pop() ===
        "tradeType"
