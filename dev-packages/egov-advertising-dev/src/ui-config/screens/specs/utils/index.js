...
export const onClickNextButton = (
  ...
  switch (queryValue) {
    case "reject":
      return `/advertising/acknowledgement?purpose=application&status=rejected&applicationNumber=${applicationNumber}&secondNumber=${secondNumber}&tenantId=${tenantId}`;
    case "cancel":
      return `/advertising/acknowledgement?purpose=application&status=cancelled&applicationNumber=${applicationNumber}&secondNumber=${secondNumber}&tenantId=${tenantId}`;
    default:
      return `/advertising/acknowledgement?purpose=approve&status=success&applicationNumber=${applicationNumber}&secondNumber=${secondNumber}&tenantId=${tenantId}`;
  }
};
...
export const onClickPreviousButton = (
  ...
  switch (queryValue) {
    case "reject":
      return `/advertising/search-preview?role=approver&status=pending_approval&applicationNumber=${applicationNumber}&tenantId=${tenantId}`;
    case "cancel":
      return `/advertising/search-preview?role=approver&status=approved&applicationNumber=${applicationNumber}&tenantId=${tenantId}`;
    default:
      return `/advertising/search-preview?role=approver&status=pending_approval&applicationNumber=${applicationNumber}&tenantId=${tenantId}`;
  }
};
...
export const pageResetAndChange = (state, dispatch, tenant) => {
  ...
  dispatch(prepareFinalObject("LicensesTemp", []));
  dispatch(setRoute(`/advertising/apply?tenantId=${tenant}`));
};
