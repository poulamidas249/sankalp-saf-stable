...
import { convertRawDataToFormConfig, convertRawDataToFormConfigNew } from "egov-ui-kit/utils/PTCommon/propertyToFormTransformer";
...
import { InstitutionAuthorityHOC, InstitutionHOC, OwnerInfoHOC, PropertyDetailsHOC, PropertyAreaDetailsHOC, RoomPropertyDetails, OwnerInformation, LandDetailsHOC, OwnershipTypeHOC, PropertyRegistrationDetails, BuildingDetailsHOC, PropertyAddressHOC, TaxCalculationHOC, UsageInformationHOC } from "./components/Forms";
...
class FormWizard extends Component {
  ...
  getSearchResultsForTempAssessee = async (queryObject, requestBody, searchURL = "/property-services/property/_search") => {

    const response = await httpRequestNew(
      "post",
      searchURL,
      "",
      queryObject,
  ...
  fetchDraftDetails = async () => {
    ...
    //   ]
    // );
    let query = { "tenantId": "km.kolkata", "searchType": "DRAFT", "tmpAsseseeNo": getQueryArg(window.location.href, "tmpAssesseeNo") };
    ...
    let searchPropertyResponse = {}
    searchPropertyResponse = await this.getSearchResultsForTempAssessee(queryObject)


    searchPropertyResponse.Properties[0].propertyDetails = {
      "status": null,
      "finalRoomDetailsValue": "bathRoom-0+bedRoom-4+hall-1+livingDining-0+conference-0+covered-0+thakurGhar-0+healthClub-0+classRoom-0+verandah-0+mezzanine-0+kitchen-0+garrage-0+stairCase-0+office-0+study-0+shed-0+terrace-0+shop-0+store-0+wallShop-0+swimingPool-0+others-0",
      "id": null,
      "wardNo": null,
      "natureOfProperty": "Residential-self",
      "typeOfUse": null,
      "buildingName": null,
      "floorNo": null,
      "flatNo": "201A",
      ...
      "coveredSpace": null,
      "openCarParking": null,
      "coveredCarParking": null,
      "openTerrace": null,
      "commonSpace": null,
      "roof": null,
      "swimingpool": "20-sqMt-small",
      "roomDetails": null
    }
    //  let roomDetailsRecord = searchPropertyResponse.Properties[0].propertyDetails
    let propertyDetailsRecord = searchPropertyResponse.Properties[0].propertyDetails


    let testArray = propertyDetailsRecord.finalRoomDetailsValue.split("+");
    let finalRoomObject = {};
    console.log(testArray);
    testArray.map((t) => {
      let splitField = t.split("-");
      if (splitField[1] !== "0") {
        finalRoomObject[splitField[0]] = splitField[1];
      }
    });
    console.log('finalRoomObject', finalRoomObject)


    let finalAreaObject = {};
  if (Object.keys(propertyDetailsRecord).includes("coveredSpace") &&  propertyDetailsRecord.coveredSpace) {
    let obj = propertyDetailsRecord.coveredSpace.split("-");
    finalAreaObject["coveredSpace"] = obj[0];
    finalAreaObject["coveredSpaceUnit"] = obj[1];
    finalAreaObject["coveredSpaceSize"] = obj[2];
  }
  if (Object.keys(propertyDetailsRecord).includes("openCarParking")  &&  propertyDetailsRecord.openCarParking) {
    let obj = propertyDetailsRecord.openCarParking.split("-");
    finalAreaObject["openCarParking"] = obj[0];
    finalAreaObject["openCarParkingUnit"] = obj[1];
    finalAreaObject["openCarParkingSize"] = obj[2];
  }
  if (Object.keys(propertyDetailsRecord).includes("coveredCarParking")  &&  propertyDetailsRecord.coveredCarParking) {
    let obj = propertyDetailsRecord.coveredCarParking.split("-");
    finalAreaObject["coveredCarParking"] = obj[0];
    finalAreaObject["coveredCarParkingUnit"] = obj[1];
    finalAreaObject["coveredCarParkingSize"] = obj[2];
  }
  if (Object.keys(propertyDetailsRecord).includes("openTerrace")  &&  propertyDetailsRecord.openTerrace) {
    let obj = propertyDetailsRecord.openTerrace.split("-");
    finalAreaObject["openTerrace"] = obj[0];
    finalAreaObject["openTerraceUnit"] = obj[1];
    finalAreaObject["openTerraceSize"] = obj[2];
  }
  if (Object.keys(propertyDetailsRecord).includes("commonSpace")  &&  propertyDetailsRecord.commonSpace) {
    let obj = propertyDetailsRecord.commonSpace.split("-");
    finalAreaObject["commonSpace"] = obj[0];
    finalAreaObject["commonSpaceUnit"] = obj[1];
    finalAreaObject["commonSpaceSize"] = obj[2];
  }
  if (Object.keys(propertyDetailsRecord).includes("roof")  &&  propertyDetailsRecord.roof) {
    let obj = propertyDetailsRecord.roof.split("-");
    finalAreaObject["roof"] = obj[0];
    finalAreaObject["roofUnit"] = obj[1];
    finalAreaObject["roofSize"] = obj[2];
  }
  if (Object.keys(propertyDetailsRecord).includes("swimingpool")  &&  propertyDetailsRecord.swimingpool) {
    let obj = propertyDetailsRecord.swimingpool.split("-");
    finalAreaObject["swimingpool"] = obj[0];
    finalAreaObject["swimingpoolUnit"] = obj[1];
    finalAreaObject["swimingpoolSize"] = obj[2];
  }

  console.log(finalAreaObject);



    delete searchPropertyResponse.Properties[0].propertyDetails
  
    let propertyDetails = [{ owners: searchPropertyResponse.Properties[0].owners }]
    searchPropertyResponse.Properties[0] = { ...searchPropertyResponse.Properties[0], propertyDetails, finalRoomObject  , propertyDetailsRecord ,finalAreaObject}

    console.log('searchPropertyResponse1234', searchPropertyResponse)
    ...
    console.log('propertyResponse1234', propertyResponse)
    const preparedForm = convertRawDataToFormConfigNew(propertyResponse); //convertRawDataToFormConfig(responseee)
    currentDraft = {
      draftRecord: {
        ...preparedForm,
        selectedTabIndex: 5,
    ...
    console.log('currentDraft1234', currentDraft)
    this.setState({
    ...
    console.log('ownerFormKeys1234', ownerFormKeys)
    const { ownerDetails, totalowners } = configOwnersDetailsFromDraft(
      ownerFormKeys,
      OwnerInformation
    );
    console.log('ownerDetails1234', ownerDetails)
    console.log('totalowners1234', totalowners)
    const activeTab =
      get(currentDraft, "draftRecord.selectedTabIndex", 0) > 5
        ? 5
    ...
    this.setState(
      ...
      () => {
        {
          if (activeTab >= 5 && !isCompletePayment) {
  ...
  componentDidMount = async () => {
    ...
    if (!getQueryValue(search, "purpose")) {
    ...
    } else if (getQueryValue(search, "purpose") == "approve") {
      await this.fetchDraftDetails();
      ...
      //   "id": "2219d820-c7e5-4054-ab2e-c132191f89b9",
      ...
      //   "tenantId": "km.kolkata",
      //   "accountId": "73da464a-0f44-444e-b4cd-73c8edce92e2",
      //   "oldPropertyId": null,
      //   "status": "INWORKFLOW",
      //   "address": null,
      //   "officeCode": "01",
      ...
      //       "ownerInfoUuid": "4d8e4ddc-7dc0-44f8-a129-b110ca6889e7",
      //       "isPrimaryOwner": null,
      ...
      //     }
      //   ],
      //   "acknowldgementNumber": "PG-AC-2022-11-03-000092",
      //   "institution": null,
      ...
      //   "auditDetails": {
      //     "createdBy": "73da464a-0f44-444e-b4cd-73c8edce92e2",
      //     "lastModifiedBy": "73da464a-0f44-444e-b4cd-73c8edce92e2",
      ...
      // prepareFinalObject('propertyRes', property);


      // prepareFinalObject('Properties[0].registrationDetails', property.registrationDetails);
      // prepareFinalObject('Properties[0].buildingdetails', property.buildingdetails);
      // prepareFinalObject('Properties[0].propertyDetails[0].owners[0]', property.owners[0]);


      // prepareFinalObject('Properties[0].premisesType', property.premisesType);
      // prepareFinalObject('Properties[0].premisesNo', property.premisesNo);
      // prepareFinalObject('Properties[0].ward', property.ward);
      // prepareFinalObject('Properties[0].street', property.street);
      // prepareFinalObject('Properties[0].address.ownerShipCategoryDropDown', property.ownershipCategory);
  ...
  getOwnerDetails = ownerType => {
    ...
    switch (ownerType) {
      ...
      case "MULTIPLEOWNERS":
        console.log('test12341234', this.state.ownerInfoArr)
        return (
  ...
  renderStepperContent = (selected, fromReviewPage) => {
    ...
    switch (selected) {
      ...
      case 2:
        return (
          <div>
            <LandDetailsHOC cardTitle="Land Details" disabled={fromReviewPage} />
            <BuildingDetailsHOC cardTitle="Building Details" disabled={fromReviewPage} />
      ...
      case 3:
        return (
          <div>
            <PropertyDetailsHOC cardTitle="Property Details" disabled={fromReviewPage} />
            <PropertyAreaDetailsHOC cardTitle="Property Area Details" disabled={fromReviewPage} />
            <RoomPropertyDetails cardTitle="Room Details" disabled={fromReviewPage} />
            {/* <UsageInformationHOC disabled={fromReviewPage} />
            {renderPlotAndFloorDetails(
              fromReviewPage,
              PlotDetails,
      ...
      case 4:
        return (
          <div>
            <PropertyRegistrationDetails disabled={fromReviewPage} />
          </div>
        );
      case 5:
        ...
        return (
          <div>
            <TaxCalculationHOC disabled={fromReviewPage} />
          </div>
        );
      case 6:
        return (
  ...
  updateIndex = index => {
    ...
    switch (selected) {
      ...
      case 2:
        ...
        //   break;
        // }
        // const isProperyAddressFormValidNew= validateForm(form.propertyAddress);
        // if (isProperyAddressFormValidNew) {
        //   callDraft(this);
        //   window.scrollTo(0, 0);
        //   this.setState({
        //     selected: index,
        ...
        if (
          window.appOverrides &&
          !window.appOverrides.validateForm("buildingDetails", form)
        ) {
          this.props.toggleSnackbarAndSetText(
            true,
            {
        ...
        let isBuildingDetailsValid = validateForm(form.buildingDetails);
        if (isBuildingDetailsValid) {
          callDraft(this);
          window.scrollTo(0, 0);
          this.setState({
            selected: index,
          ...
          displayFormErrorsAction("buildingDetails");
        }
        break;
      case 3:
        // if (
        ...
        if (
          window.appOverrides &&
          !window.appOverrides.validateForm("propertyAreaDetails", form)
        ) {
          this.props.toggleSnackbarAndSetText(
            true,
            {
        ...
        let isRoomDetailsValid = validateForm(form.propertyAreaDetails);
        if (isRoomDetailsValid) {
          ...
          displayFormErrorsAction("propertyAreaDetails");
        }
        break;
      ...
      case 4:
        if (
          window.appOverrides &&
          !window.appOverrides.validateForm("propertyRegistrationDetails", form)
        ) {
          this.props.toggleSnackbarAndSetText(
            true,
            {
        ...
        const isPropertyRegistrationDetailsFormValid = validateForm(form.propertyRegistrationDetails);
        if (isPropertyRegistrationDetailsFormValid) {
          callDraft(this);
          window.scrollTo(0, 0);
          this.setState({
            selected: index,
          ...
          displayFormErrorsAction("propertyRegistrationDetails");
        }
        break;
      case 5:
        if (
          window.appOverrides &&
          !window.appOverrides.validateForm("taxCalculation", form)
        ) {
          this.props.toggleSnackbarAndSetText(
            true,
            {
        ...
        const istaxCalculationHOCFormValid = validateForm(form.taxCalculation);
        if (istaxCalculationHOCFormValid) {
          if (parseInt(form.taxCalculation.fields.commAV.value) < 0 || parseInt(form.taxCalculation.fields.commAV.value) > parseInt(form.taxCalculation.fields.AV.value)) {
            this.props.toggleSnackbarAndSetText(
              true,
              {
                labelName: "Commercial AV must be less than AV",
                labelKey: "Commercial AV must be less than AV"
              },
              "error"
            );
            break;
          ...
          displayFormErrorsAction("taxCalculation");
        }
        break;
      // window.scrollTo(0, 0);
      // const newDocs = {};
      ...
      case 6:
        ...
        let isApprove = getQueryValue(search1, "purpose") == 'approve';
  ...
  createAndUpdate = async (index, action) => {
    ...
    set(
      prepareFormData,
      "Properties[0].propertyDetails[0].tenantId",
      "km"
    );
  ...
  render() {
    ...
    const fromReviewPage = selected === 6;
