...
import {
  ...
  getCreatePropertyResponse,
  prefillPTDocuments,
  setOldPropertyData,
} from "egov-ui-kit/config/forms/specs/PropertyTaxPay/propertyCreateUtils";
import {
  setRoute,
...
import {
  ...
  hideSpinner,
  prepareFormData as prepareFormDataAction,
  showSpinner,
  toggleSpinner,
...
import {
  ...
  getPurpose,
  propertySubmitAction,
  PROPERTY_FORM_PURPOSE,
  getPTApplicationTypes,
} from "egov-ui-kit/utils/PTCommon/FormWizardUtils/formUtils";
...
import ReviewFormInspection from "./components/ReviewFormInspection";
import WizardComponentInspection from "./components/WizardComponentInspection";
import "./index.css";
import { getDocumentTypes } from "./utils/mdmsCalls";
import {
  generalMDMSDataRequestObj,
  getGeneralMDMSDataDropdownName,
} from "egov-ui-kit/utils/commons";

class InspectionFormWizard extends Component {
  state = {
    dialogueOpen: false,
  ...
  fetchDraftDetails = async (draftId, isReassesment, draftUuid) => {
    ...
    try {
      ...
      searchPropertyResponse = getCreatePropertyResponse(
      ...
      this.props.prepareFinalObject(
        "newProperties",
        searchPropertyResponse.newProperties
      );
      if (
  ...
  componentDidMount = async () => {
    ...
    let isInspectionSummary = getQueryValue(search, "isInspection") == "yes";
      ...
      this.props.fetchAssessments([
        {
          ...
          value: getQueryValue(search, "assessmentId"),
        },
    ...
    // } else if (
    //   getQueryValue(search, "purpose") == "update" ||
    //   getQueryValue(search, "purpose") == "assess" ||
    //   getQueryValue(search, "purpose") == "reassess"
    // ) {
    //   prepareFinalObject(
    //     "Properties",
    //     this.props.common.prepareFormData.Properties
    //   );
    // }
  ...
  getOwnerDetails = (ownerType) => {
    ...
    switch (ownerType) {
      case "SINGLEOWNER":
        return <OwnerInfoHOCAV disabled={disableOwner} />;
      case "MULTIPLEOWNERS":
        return (
      ...
      case "INSTITUTIONALGOVERNMENT":
        return (
          <div>
            <InstitutionHOCAV disabled={disableOwner} />
            <InstitutionAuthorityHOCAV
              cardTitle={
                <Label
  ...
  renderStepperContent = (selected, fromReviewPage) => {
    ...
    switch (selected) {
      case 0:
        return (
          <div>
            <PropertyAddressHOCAV disabled={fromReviewPage} />
          </div>
        );
      case 1:
        ...
        return (
          <div>
            <OwnershipTypeHOCAV disabled={disableOwner} />
            {getOwnerDetails("SINGLEOWNER")}
          </div>
        );
      case 2:
        return (
          <div>
            <LandDetailsHOCAV
              cardTitle="Land Details"
              disabled={fromReviewPage}
            />
            <BuildingDetailsHOCAV
              cardTitle="Building Details"
              disabled={fromReviewPage}
            />
      ...
      case 3:
        return (
          <div>
            <PropertyRegistrationDetailsAV disabled={fromReviewPage} />
          </div>
        );
      case 4:
        ...
        return (
          <div>
            <TaxCalculationHOCAV disabled={fromReviewPage} />
          </div>
        );
      case 5:
        return (
          <div className="review-pay-tab">
            <ReviewFormInspection
              properties={capturedAVData}
  ...
  getButtonLabels(index) {
    ...
    if (index == 5) {
      buttonLabel = "Generate 184 Notice";
    } else if (index == 6) {
  ...
  getHeader(selected, search, PTUID) {
    ...
    switch (selected) {
      ...
      case 4:
      case 5:
        headerObj.subHeaderValue = formWizardConstants[purpose].isSubHeader
          ? propertyId
          : "";
        headerObj.headerValue = formWizardConstants[purpose].isFinancialYear
          ? `(${assessmentYear})`
          : "";
  ...
  resetForm = () => {
    const { form, removeForm, prepareFormDataAction, prepareFinalObject } =
      this.props;
  ...
  updateIndex = (index) => {
    const { pay, estimate, createReceipt, createAndUpdate, onPayButtonClick } =
      this;
    ...
    switch (selected) {
      ...
      case 0:
        ...
        //     true,
        //     {
        //       labelName: "Property Address validations failed!",
        //       labelKey: "Property Address validations failed!",
        //     },
        //     "error"
        //   );
        //   break;
        ...
        const isPropertyAddressFormValid = validateForm(form.propertyAddress);

        if (isPropertyAddressFormValid) {
          callDraft(this);
          window.scrollTo(0, 0);
          this.setState({
            selected: index,
          ...
          this.props.toggleSnackbarAndSetText(
            true,
            {
              labelName: "Form validations failed!",
              labelKey: "Form validations failed!",
            },
            "warning"
          );
        }
      ...
      case 1:
        ...
        //       const isInstitutionDetailsFormValid =
        //         validateForm(institutionDetails);
        ...
        //       }
        //       const isInstitutionAuthorityFormValid =
        //         validateForm(institutionAuthority);
        ...
        const isOwnershipFormValid = validateForm(form.ownerInfo);

        if (isOwnershipFormValid) {
          callDraft(this);
          window.scrollTo(0, 0);
          this.setState({
            selected: index,
      ...
      case 2:
        const isLandLandFormValid = validateForm(form.landDetails);
        const isLandBuildingFormValid = validateForm(form.buildingDetails);

        if (isLandLandFormValid && isLandBuildingFormValid) {
          callDraft(this);
          window.scrollTo(0, 0);
          this.setState({
            selected: index,
      ...
      //   break;
      // }
      // const isBuildingDetailsValid = validateForm(form.buildingDetails);
      ...
      case 3:
        ...
        const isPropertyRegistrationDetailsFormValid = validateForm(
          form.propertyRegistrationDetails
        );
      ...
      case 5:
        ...
        // }
        let isInspection = getQueryValue(search1, "isInspection") == "yes";
        // if inspection
        if (isInspection) {
          this.setState({ nextButtonEnabled: false });
          showSpinner();
      ...
      case 6:
        ...
        this.props.history.push(
          `./../egov-common/pay?consumerCode=${propertyId}&tenantId=${tenantId}&businessService=PT`
        );
        // this.setState(
        //   {
        //     selected: index,
  ...
  callGetBill = async (
    ...
    const { location } = this.props;
    const { isFullPayment, totalAmountToBePaid, estimation, valueSelected } =
      this.state;
    const { search } = location;
    const isCompletePayment = getQueryValue(search, "isCompletePayment");
    const queryObj = [
  ...
  createReceipt = async (Bill = []) => {
    ...
    const { assessmentNumber, propertyId, tenantId, assessmentYear } =
      propertyDetails;
      ...
      this.estimate().then((estimateResponse) => {
        if (estimateResponse) {
          ...
          if (isReassesment && Assessments && Assessments.length > 0) {
            adhocExemptionAmt =
              get(Assessments[0], "additionalDetails.adhocExemption", 0) || 0;
            adhocPenaltyAmt =
              get(Assessments[0], "additionalDetails.adhocPenalty", 0) || 0;
          }
          estimateResponse.Calculation[0].initialAmount =
            estimateResponse.Calculation[0].totalAmount;
          estimateResponse.Calculation = getFormattedEstimate(
            estimateResponse.Calculation,
            adhocPenaltyAmt,
            adhocExemptionAmt
          );
  ...
  // handler for add property button
  createAndUpdate = async (index, action) => {
    const { selected, formValidIndexArray, prepareFinalObject } = this.state;
    const financialYearFromQuery = getFinancialYearFromQuery();
    let { form, common, location, hideSpinner, preparedFinalObject } =
      this.props;
    const { search } = location;
    const propertyId = getQueryValue(search, "propertyId");
    const assessmentId = getQueryValue(search, "assessmentId");
    const isInspection = getQueryValue(search, "isInspection");
    ...
    set(prepareFormData, "Properties[0].propertyDetails[0].tenantId", "km");
  ...
  // Generate 184 notice handler
  onPayButtonClick = async () => {
    const state = store.getState();
    let propertiesData = get(
      state.screenConfiguration,
      "preparedFinalObject.Properties"
    );
    let userInputOnSearchForm = get(
      state.screenConfiguration,
      "preparedFinalObject.capturedProposed"
    );

    if (propertiesData && propertiesData.length > 0) {
      let data = propertiesData[0];
      let requestBody = {
        Property: {
          assesseeNo: data.assesseeNo,
          inspectionType: userInputOnSearchForm.grIr,
          ward: data.ward,
          street: data.street,
          premisesNo: data.premisesNo,
          premisesType: data.premisesType,
          registrationDetails: {
            dag: data.registrationDetails.dag,
            khatian: data.registrationDetails.khatian,
            mouza: data.registrationDetails.mouza,
            queryNo: data.registrationDetails.queryNo,
            queryYear: data.registrationDetails.queryYear,
            deedNo: data.registrationDetails.deedNo,
            deedYear: data.registrationDetails.deedYear,
            location: data.registrationDetails.location,
            ro: data.registrationDetails.ro,
            book: data.registrationDetails.book,
          },
          owners: [
            {
              name: data.owners[0].userName,
              mobileNumber: data.owners[0].mobileNumber,
              fatherOrHusbandName: data.owners[0].fatherOrHusbandName,
              emailId: data.owners[0].emailId,
              permanentAddress: data.owners[0].permanentAddress,
              relationship: data.owners[0].relationship,
              pin: data.owners[0].pincode,
              policeStation: data.owners[0].policeStation,
              postOffice: data.owners[0].postOffice,
              gender: data.owners[0].gender,
              ownerType: data.ownershipType,
            },
          ],
          ownershipCategory: data.ownershipCategory,
          buildingDetails: {
            buildingType: data.buildingdetails.buildingType,
            occupancyStatus: data.buildingdetails.occupancyStatus,
            numberOfStories: data.buildingdetails.numberOfStories,
            plotArea: data.buildingdetails.plotArea,
            coveredArea: data.buildingdetails.coveredArea,
            parkingArea: data.buildingdetails.parkingArea,
            commonArea: data.buildingdetails.commonArea,
            characterofPremises: data.buildingdetails.characterofPremises,
            acre: data.buildingdetails.acre,
          },
          taxDetails: {
            uaaConverted: data.taxdetails.uaaConverted,
            safNo: data.taxdetails.safNo,
            cappingMethod: data.taxdetails.cappingMethod,
            uncappedTax: data.taxdetails.uncappedTax,
            uncappedHbt: data.taxdetails.uncappedHbt,
            status: data.taxdetails.status,
            id: data.taxdetails.id,
            quarter: data.taxdetails.quarter,
            effectiveQtr: data.taxdetails.effectiveQtr,
            proposedQtr: data.taxdetails.proposedYear.concat(
              data.taxdetails.proposedQtr
            ),
            av: data.taxdetails.av,
            commAv: data.taxdetails.commAv,
            reasonableRent: data.taxdetails.reasonableRent,
            rate: data.taxdetails.rate,
            commRate: data.taxdetails.commRate,
            hbtAmount: data.taxdetails.hbtAmount,
            surcharge: data.taxdetails.surcharge,
            grossAmount: data.taxdetails.grossAmount,
            rebateAmount: data.taxdetails.rebateAmount,
            quateryAmount: data.taxdetails.quateryAmount,
            paybleAmount: data.taxdetails.paybleAmount,
            netAmount: data.taxdetails.netAmount,
            premisesType: data.taxdetails.premisesType,
            appdFlag: data.taxdetails.appdFlag,
          },
          notice: {
            acceptBy: "EMPLOYEE",
          },
      ...
      // let link = `/pt-inspection/acknowledgementGRIR?purpose=apply&status=success&applicationNumber=${data.assesseeNo}&noticeNo`;
  ...
  render() {
    const { renderStepperContent, onPayButtonClick, closeDeclarationDialogue } =
      this;
    const {
      selected,
      ownerInfoArr,
    ...
    const fromReviewPage = selected === 5;
