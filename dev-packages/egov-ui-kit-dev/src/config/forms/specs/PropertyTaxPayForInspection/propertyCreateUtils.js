  ...
  if (documentsUploadRedux && Object.keys(documentsUploadRedux) && Object.keys(documentsUploadRedux).length) {
    ...
    Object.keys(documentsUploadRedux).map((key) => {
      if (
        ...
        documentsUploadRedux[key].documents[0].fileStoreId
      ) {
...
export const getCreatePropertyResponse = (createPropertyResponse) => {
  ...
  // createPropertyResponse.Properties[0] &&
  //   createPropertyResponse.Properties[0].documents &&
  //   createPropertyResponse.Properties[0].documents.length &&
  //   createPropertyResponse.Properties[0].documents.reverse();
  console.log("convert => ", convertToOldPTObject(createPropertyResponse));
  ...
  if (documentsUploadRedux && typeof documentsUploadRedux === "object") {
    if (Object.keys(documentsUploadRedux) && Object.keys(documentsUploadRedux).length) {
      ...
      Object.keys(documentsUploadRedux).map((key) => {
        ...
        if (
          documentsUploadRedux[key].documents &&
          documentsUploadRedux[key].documents[0].fileUrl &&
      ...
      return {
        ...
        name:
          (fileUrlPayload &&
            ...
            decodeURIComponent(getFileUrl(fileUrlPayload[item.fileStoreId]).split("?")[0].split("/").pop().slice(13))) ||
          `Document - ${index + 1}`,
      };
    });
...
export const setOldPropertyData = async (search, prepareFinalObject) => {
  ...
  let searchPropertyResponse = await httpRequest("property-services/property/_search", "_search", [
    {
      key: "tenantId",
      value: tenantId,
    },
