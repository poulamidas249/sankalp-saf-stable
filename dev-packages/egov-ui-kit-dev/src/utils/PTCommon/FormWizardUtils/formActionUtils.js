...
export const createProperty = async (Properties, action, props, isModify, preparedFinalObject) => {
    ...
    console.log('preparedFinalObject1234', preparedFinalObject)
    ...
    if (!isEditInWorkflow) {
        ...
        if (action == '_create') {
            ...
            propertyPayload.tmpAssesseeNo = getQueryValue(search, "tmpAssesseeNo") ? getQueryValue(search, "tmpAssesseeNo") : ""
            propertyPayload.inspectionType = "IR",
                propertyPayload.notice = {
                    "acceptBy": "EMPLOYEE"
                }
    ...
    process.env.REACT_APP_NAME === "Citizen" ? null : propertyPayload.notice = {
        "acceptBy": "EMPLOYEE"
    };

    let TaxDetialsBody = get(preparedFinalObject, "taxDetailsResponse", {})


    propertyPayload.propertyDetails = {}

    propertyPayload.propertyDetails = { ...propertyPayload.propertyDetailsNew ,  "roomDetails" : propertyPayload.roomPropertyDetails }


    delete propertyPayload.propertyDetailsNew
    delete propertyPayload.roomPropertyDetails
    delete propertyPayload.propertyAreaDetails


    propertyPayload.characterofPremises = propertyPayload.buildingdetails ? (propertyPayload.buildingdetails.characterofPremises ? propertyPayload.buildingdetails.characterofPremises : null) : null

    propertyPayload.ownershipCategory = propertyPayload.ownerShipCategoryDropDown

    delete propertyPayload.address

    propertyPayload.buildingDetails = { ...propertyPayload.buildingdetails, ...propertyPayload.buildingDetails }

    delete propertyPayload.buildingdetails

    propertyPayload.tenantId = "km.kolkata",

        propertyPayload.taxdetails = TaxDetialsBody

    delete propertyPayload.usageCategory

    console.log('propertyPayload1324', propertyPayload)
    ...
    if (propertyResponse && propertyResponse.Properties && propertyResponse.Properties.length) {
        if (process.env.REACT_APP_NAME === "Citizen") {
            if (propertyResponse.Properties[0].acknowldgementNumber) {
                const propertyId = get(propertyResponse, "Properties[0].acknowldgementNumber", '');
            ...
            if (propertyResponse.Properties[0].assesseeNo) {
                console.log('empUpdate', Properties[0].assesseeNo)
                const propertyId = get(propertyResponse, "Properties[0].assesseeNo", '');
                const tenantId = get(propertyResponse, "Properties[0].tenantId", '');
                const acknowldgementNumber = get(propertyResponse, "Properties[0].acknowldgementNumber", '');
                // Navigate to success page
                if (action == '_create') {
                    routeToAcknowledgement(PROPERTY_FORM_PURPOSE.CREATE, 'success', propertyId, tenantId, propertyId);
                } else {
                    routeToAcknowledgement(PROPERTY_FORM_PURPOSE.UPDATE, 'success', propertyId, tenantId, propertyId);
                }
            }
        }
